...
PROC SQL;
	CREATE TABLE APR_BOOK AS
	SELECT * FROM ALL_APPS_3 WHERE ENTYRMONTH IN (201804);
QUIT; /* CHANGE */

*** SUBSET FOR NON 'NB' AND 'FB' CUSTOMER TYPE ------------------- ***;
PROC SQL;
	CREATE TABLE ALL_APPS_4 AS
	SELECT ENTYRMONTH, BRACCTNO, NETLOANAMOUNT, CLASSTRANSLATION 
		AS CLASSTRANSLATION1 
	FROM APR_BOOK 
...
PROC SQL;
	CREATE TABLE ABC AS 
	SELECT ENTYRMONTH, COUNT(*) FROM ALL_APPS_4 GROUP BY 1;
QUIT;
...
PROC SQL;
	CREATE TABLE ALL_APPS_5 AS 
	SELECT A.*, B.SSNO1, B.NETLOANAMOUNT AS NETLOANAMOUNT_B, B.OWNBR,
		   B.CLASSID, B.CLASSTRANSLATION, B.POCD, B.PLCD, B.PLDATE,
		   B.POFFDATE, B.LNAMT, B.FINCHG, B.PRLNNO, B.AMTPAIDLAST, 
		   B.OLDACCTNO, B.ORGBR, B.LOANTYPE AS LOANTYPE_PI, B.ORGST 
	FROM ALL_APPS_4 A 
...
PROC SQL;
	CREATE TABLE ALL_APP_7 AS
	SELECT *, LENGTH(PRLNNO) AS PRLN_LENGTH,
		CASE WHEN ORGST IN ('OK','SC') AND 
				  LENGTH(PRLNNO) = 9 THEN '0' || PRLNNO
			 WHEN LENGTH(PRLNNO) = 1 THEN NEW_OWNBR || 
				SUBSTR(('0000000' || PRLNNO), 1, 6)
			 WHEN LENGTH(PRLNNO) = 2 THEN NEW_OWNBR || 
				SUBSTR(('000000' || PRLNNO), 1, 6)
			 WHEN LENGTH(PRLNNO) = 3 THEN NEW_OWNBR ||
				SUBSTR(('00000' || PRLNNO), 1, 6)
			 WHEN LENGTH(PRLNNO) = 4 THEN NEW_OWNBR ||
				SUBSTR(('0000' || PRLNNO), 1, 6)
			 WHEN LENGTH(PRLNNO) = 5 THEN NEW_OWNBR ||
				SUBSTR(('000' || PRLNNO), 1, 6)
			 WHEN LENGTH(PRLNNO) = 6 THEN NEW_OWNBR ||
				SUBSTR(('00' || PRLNNO), 1, 6)
			 WHEN LENGTH(PRLNNO) = 7 THEN NEW_OWNBR ||
				SUBSTR(('0' || PRLNNO), 1, 6)
			 WHEN LENGTH(PRLNNO) = 8 THEN NEW_OWNBR ||
				SUBSTR(('' || PRLNNO), 1, 6)
...
PROC SQL;
	CREATE TABLE ALL_APPS_8 AS 
	SELECT A.ENTYRMONTH, A.BRACCTNO AS RENEW_BRACCTNO, 
		   A.NETLOANAMOUNT_B AS RENEW_NETLONAMOUNT, A.CLASSTRANSLATION,
		   A.AMTPAIDLAST, A.ORGST, A.OLD_BRACCTNO AS OLD_BRACCTNO1,
		   B.BRACCTNO AS OLD_BRACCTNO, B.SRCD AS OLD_SRCD,
		   B.CLASSID  AS OLD_CLASSID, B.AMTPAIDLAST AS OLD_AMTPAIDLAST
	FROM ALL_APP_7 A 
...
PROC EXPORT 
	DATA = ALL_APPS_9 
	OUTFILE = "&MAIN_DIR\ALL_APPS_9.xlsx" 
	DBMS = EXCEL 
	REPLACE;
RUN;
...
PROC SQL;
	CREATE TABLE ABC AS 
	SELECT ENTYRMONTH, COUNT(*), SUM(RENEW_AMT) 
	FROM ALL_APPS_9 
...
PROC SQL;
	CREATE TABLE ALL_APPS_10 AS 
	SELECT A.*, B.OLD_BRACCTNO1, B.RENEW_BRACCTNO,
		   B.RENEW_NETLONAMOUNT, B.OLD_AMTPAIDLAST, B.RENEW_AMT
	FROM ALL_APPS_3 A 
	LEFT JOIN ALL_APPS_9 B ON A.BRACCTNO = B.RENEW_BRACCTNO; 
QUIT;
